...
static Menu_Param menu_param = {0, 0.8, 0.1, 0.01};
...
void MENU_Update(void)
{
    ...
    if(current_item == MENU_ON_OFF)
        ...
        sprintf(buf, "Run:%s ", menu_param.on_off ? "ON " : "OFF");
    OLED_ShowString(1, 1, buf);  // 移除(uint8_t*)强制转换
    ...
    OLED_ShowString(2, 1, buf);  // 移除(uint8_t*)强制转换
    ...
    OLED_ShowString(3, 1, buf);  // 移除(uint8_t*)强制转换
    ...
    OLED_ShowString(4, 1, buf);  // 移除(uint8_t*)强制转换
}
